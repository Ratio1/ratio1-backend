...
var (
	serviceTestDatabaseOnce sync.Once
...
func requireServiceTestDatabase(t *testing.T) {
	...
	serviceTestDatabaseOnce.Do(func() {
		...
		serviceTestDatabaseErr = storage.Migrate(context.Background(), databaseConfig)
		if serviceTestDatabaseErr != nil {
			return
		}
		config.Config.Database = databaseConfig
		storage.Connect()
	})
	require.NoError(t, serviceTestDatabaseErr)
}
