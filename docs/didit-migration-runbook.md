...
1. Merge and deploy the backend artifact with `VERIFICATION_PROVIDER=sumsub`; keep Didit session creation inactive.
2. Inject the production database variables from the secret store and run the immutable release artifact's one-shot migration command before starting that same API version:

   ```bash
   EE_EVM_NET=mainnet \
     /path/to/immutable-release/ratio1-api \
     --general-config /path/to/immutable-release/config/ \
     migrate
   ```

   Use the exact binary or container image digest that will serve production traffic; do not rebuild a separate migration binary. A zero exit and `database migration completed` are hard release gates. The command runs the duplicate-email preflight, applies the schema, upgrades KYC email enforcement to the `uni_kycs_email` unique constraint, and verifies all required tables and indexes. Normal API startup and the cutover CLI deliberately do not run migrations.
3. Confirm with a read-only database query that `kycs.verification_provider`, `verification_sessions`, `verification_webhook_events`, and `verification_notifications` exist, and that `uni_kycs_email` is reported as `CREATE UNIQUE INDEX`:
   ...
   WHERE schemaname = CURRENT_SCHEMA()
     AND indexname = 'uni_kycs_email';
   ```

4. Publish and lock the production KYC and KYB workflows and record their exact versions.
5. Confirm all KYC/KYB features and questionnaire versions. KYC billing fields remain questionnaire-backed; the KYB questionnaire contains Source-of-Funds fields only.
6. Confirm document, residence and company-country restrictions.
7. In both sandbox and production KYC workflows, use Didit's **Apply age of majority**, review every country and state/region override, and configure below-minimum results to **Decline**. Sandbox workflow version 1 was reverified on 2026-08-14 with the jurisdiction-specific matrix and `DECLINE`; the locked production workflow must still be verified independently before activation.
8. In the native KYB Registry Fields configuration, confirm company name, country and region are collected; set `legal_address` and `tax_number` to Required and `vat_number` to Required (Didit enforces it conditionally for EU companies). Do not duplicate these fields in the Source-of-Funds questionnaire.
9. Complete Sandbox acceptance and production Try Webhook tests.
10. Confirm a stable public HTTPS webhook URL and fixed dApp callback URL.
11. Confirm provider billing balance, ongoing-monitoring settings and alerts.
12. Announce a verification maintenance window that prevents KYC creation and mutation during the transaction.
